...
totals:{ganhos:10157.73,ganhosLiquidos:7709.84,saftBruto:8227.97,saftIliquido:7761.67,saftIva:466.30,despesas:2447.89,faturaPlataforma:262.94,dac7TotalPeriodo:7755.16,dac7Q1:0,dac7Q2:0,dac7Q3:0,dac7Q4:7755.16},
crossings:{discrepanciaCritica:2184.95,percentagemOmissao:89.26,ivaFalta:502.54,ivaFalta6:131.10,btor:2447.89,btf:262.94,discrepanciaSaftVsDac7:2402.57,percentagemSaftVsDac7:23.65,agravamentoBrutoIRC:2184.95,ircEstimado:458.84,impactoSeteAnosMercado:0,impactoMensalMercado:0,impactoAnualMercado:0,discrepancia5IMT:0},
twoAxis:{revenueGap:0,expenseGap:2184.95,revenueGapActive:false,expenseGapActive:true},
verdict:{level:{pt:"RISCO CRÍTICO · INFRAÇÃO DETETADA",en:"CRITICAL RISK · INFRACTION DETECTED"},key:"critical",color:"#ff0000",description:{pt:"Evidência de subcomunicação de proveitos (DAC7) e omissão grave de faturação de custos (89,26%). A plataforma retém valores sem a devida titulação fiscal, prejudicando o direito à dedução de IVA e inflacionando a base de IRC do contribuinte.",en:"Evidence of income under-reporting (DAC7) and serious cost invoicing omission (89.26%). The platform retains amounts without proper tax documentation, prejudicing the right to VAT deduction and inflating the taxpayer's IRC base."},percent:"89.26%"},
...
if(el){el.textContent=value;_auditLog.push({id:id,value:value,ts:new Date().toISOString()});return true;}
_auditLog.push({id:id,value:value,ts:new Date().toISOString(),warn:'ELEMENT_NOT_FOUND'});return false;
}
function _setWithXPathFallback(id,value,xpathSearchText){
var el=document.getElementById(id);
if(el){el.textContent=value;_auditLog.push({id:id,value:value,ts:new Date().toISOString(),method:'byId'});return true;}
var xpath="//*[contains(text(),'"+xpathSearchText.replace(/'/g,"\\'")+"')]";
var result=document.evaluate(xpath,document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null);
var target=result.singleNodeValue;
if(target&&target.nextElementSibling){target.nextElementSibling.textContent=value;_auditLog.push({id:id,value:value,ts:new Date().toISOString(),method:'xpath',xpathSearch:xpathSearchText});return true;}
_auditLog.push({id:id,value:value,ts:new Date().toISOString(),warn:'XPATH_FAILED',xpathSearch:xpathSearchText});return false;
}
...
var semesterContainer=document.getElementById('semestreSelectorContainer')||document.querySelector('.semester-container')||document.getElementById('trimestralSelectorContainer');
...
if(semSelect){semSelect.value='2';semSelect.style.display='inline-block';}
...
if(periodoSelect){
var originalPeriodoChange=periodoSelect.onchange;
periodoSelect.addEventListener('change',function(e){
if(periodoSelect.value==='anual'){
periodoSelect.value='semestral';
enforceSemester();
}
if(typeof originalPeriodoChange==='function')originalPeriodoChange.call(periodoSelect,e);
});
}
...
style.id=styleId;
style.textContent=`
.kpi-card.dac7-card, [id*="dac7"] .kpi-card, .dac7-container .kpi-card, .dac7-box {
...
.kpi-card.dac7-card .kpi-value, [id*="dac7"] .kpi-value, .dac7-box .value {
order: 2 !important;
...
.kpi-card.dac7-card .kpi-label, [id*="dac7"] .kpi-label, .dac7-box .label {
order: 1 !important;
text-align: center !important;
...
<th style="padding:8px; background:#e0e0e0;">Δ vs DAC7</th></tr>
</thead>
<tbody>
...
if(anoFiscalSelect){anoFiscalSelect.value=String(sys.selectedYear);anoFiscalSelect.dispatchEvent(new Event('change',{bubbles:true}));}
var periodoSelect=document.getElementById('periodoAnalise');
if(periodoSelect){periodoSelect.value=sys.selectedPeriodo;periodoSelect.dispatchEvent(new Event('change',{bubbles:true}));
var triContainer=document.getElementById('trimestralSelectorContainer');
if(triContainer){triContainer.style.display='none';triContainer.classList.remove('show');}}
var semestreSelect=document.getElementById('semestreSelector');
if(semestreSelect){semestreSelect.value=String(sys.selectedSemestre);semestreSelect.dispatchEvent(new Event('change',{bubbles:true}));}
...
else{var total=sys.analysis.evidenceIntegrity.length;
...
if(el){el.textContent=_fmt(item.value);_auditLog.push({id:item.id,value:_fmt(item.value),ts:new Date().toISOString()});
...
dac7Items.forEach(function(item){
var el=document.getElementById(item.id);
if(el){var parentCard=el.closest('.kpi-card');if(parentCard)parentCard.style.display='flex';}
});
},200);
...
console.log('[UNIFED-PURE] ✅ Injeção concluída. Campos escritos: '+_written+' | IDs não encontrados no DOM: '+_missing+' | Sessão: '+sys.sessionId);
