...
// ============================================================================
// MÓDULO PATCH v13.11.16-PURE · RTF-UNIFED-2026-0406-001
// PROTOCOLO ANTI-TAMPERING REFORÇADO (OVERLAY) + CONSTRUTOR DOM ATÓMICO
// ============================================================================
// [R-N06]: Camada adicional de blindagem contextual sobre cards de evidência.
// [R-N07]: _buildRiscoPanelDOM() usa document.createElement (sem innerHTML).
// [R-N08]: Integração com UNIFEDEventBus para injeção atómica pós-DOM_READY.
// ============================================================================
(function _nexusOverlayPatch() {
    'use strict';

    // ── R-N06: Anti-Tampering de Contexto em Áreas de Evidência ──────────────
    document.addEventListener('contextmenu', function(e) {
        var target = e.target;
        var isProtected = (
            (target.closest && (target.closest('.pure-card') || target.closest('.console-section'))) ||
            (target.parentNode && target.parentNode.className &&
             (String(target.parentNode.className).indexOf('pure-card') !== -1 ||
              String(target.parentNode.className).indexOf('console-section') !== -1))
        );
        if (isProtected) {
            e.preventDefault();
            console.warn('[NEXUS·SECURITY] Bloqueio de inspeção contextual em área de evidência material.');
        }
    }, false);

    // ── R-N07: Construtor DOM atómico para Painel ATF de Risco Futuro ─────────
    function _buildRiscoPanelDOMv2() {
        var lang = window.currentLang || 'pt';

        var container = document.createElement('div');
        container.id = 'nexus-atf-risk-panel';
        container.className = 'pure-card';
        container.style.cssText = 'border-top: 2px solid var(--pure-accent, #00E5FF); margin-top: 20px;';

        var title = document.createElement('div');
        title.className = 'pure-card-title';
        title.setAttribute('data-pt', '📈 PROJEÇÃO DE RISCO (6 MESES)');
        title.setAttribute('data-en', '📈 RISK PROJECTION (6 MONTHS)');
        title.textContent = lang === 'en' ? '📈 RISK PROJECTION (6 MONTHS)' : '📈 PROJEÇÃO DE RISCO (6 MESES)';

        var value = document.createElement('div');
        value.className = 'pure-card-value';
        value.style.color = 'var(--pure-warn, #EF4444)';
        value.setAttribute('data-pt', 'ELEVADO');
        value.setAttribute('data-en', 'HIGH');
        value.textContent = lang === 'en' ? 'HIGH' : 'ELEVADO';

        var badge = document.createElement('div');
        badge.className = 'pure-subject-label';
        badge.style.marginTop = '8px';
        badge.textContent = 'ALGORITMO ATF-2SIGMA ACTIVE · v13.11.16-PURE';

        var atfSystem = window.UNIFEDSystem;
        if (atfSystem && atfSystem.analysis && atfSystem.analysis.crossings) {
            var pct = atfSystem.analysis.crossings.percentagemOmissao || 0;
            if (pct > 0) {
                var pctEl = document.createElement('div');
                pctEl.className = 'pure-subject-label';
                pctEl.style.cssText = 'color: var(--pure-warn, #EF4444); margin-top: 4px; font-weight: 700;';
                pctEl.textContent = 'DESVIO PROJETADO: ' + pct.toFixed(2) + '%';
    ...
    // ── R-N08: Injeção atómica via EventBus após DOM_READY ──────────────────
    function _injectRiscoPanelV2() {
        // Verificar se já foi injectado (idempotência)
        if (document.getElementById('nexus-atf-risk-panel')) {
            console.info('[NEXUS·PATCH] Painel ATF já injectado — ignorando duplicação.');
            return;
        }
        var wrapper = document.getElementById('pureDashboardWrapper');
        if (!wrapper) { return; }
        var pureSection = wrapper.querySelector('#pureDashboard') || wrapper;
        if (pureSection) {
            pureSection.appendChild(_buildRiscoPanelDOMv2());
            console.info('[NEXUS·PATCH] ✅ Painel de Risco ATF v2 injectado (createElement — sem innerHTML).');
        }
    }

    // Aguardar DOM_READY via EventBus ou fallback nativo
    if (window.UNIFEDEventBus) {
        window.UNIFEDEventBus.waitFor('UNIFED_DOM_READY', 6000).then(function() {
            _injectRiscoPanelV2();
        }).catch(function() {
            // Fallback: tentar após EVIDENCE_LOADED
            window.UNIFEDEventBus.on('UNIFED_EVIDENCE_LOADED', function() {
                _injectRiscoPanelV2();
            });
        });
    } else {
        window.addEventListener('UNIFED_DOM_READY', function() {
            _injectRiscoPanelV2();
        }, { once: true });
    }

    // Expor para chamada externa se necessário
    window._nexusInjectRiscoPanel = _injectRiscoPanelV2;

    console.info('[NEXUS·PATCH] ✅ Módulo Overlay v13.11.16-PURE activo (R-N06, R-N07, R-N08).');
})();
